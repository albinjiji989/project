...
$selQry = "select * from tbl_nutritionchat c inner join tbl_nutrition d on (c.from_nutrition_id=d.nutrition_id) or (c.to_nutrition_id=d.nutrition_id) inner join tbl_user u on (u.user_id=c.from_user_id) or (u.user_id=c.to_user_id) where (c.from_nutrition_id='" . $_SESSION["nid"] . "' or c.to_nutrition_id='" . $_SESSION["nid"] . "') and u.user_id='".$_GET['id']."' order by c.chat_id";
