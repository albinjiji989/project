...
$sel = "select * from tbl_nutrition where nutrition_id='" . $_GET["id"] . "'";
$res =$con->query($sel);
$row = $res->fetch_assoc();
?>
<!DOCTYPE html>
...
<body>
                ...
                <div class="nav-start">
                    <div class="recipient-block">
                        <div class="avatar-container">
                            <img class="user-avatar" src="../Assets/Files/Nutrition/<?php echo $row["nutrition_photo"] ?>" alt="">
                        </div>
                        <div class="username">
                            <span><?php echo $row["nutrition_name"] ?></span>
                        </div>
                    </div>
                </div>
                <div class="nav-end">
    ...
    function sendChat(id) {
        ...
        if (chat.length <= 35) {
            $.ajax({
                url: "../Assets/AjaxPages/NutritionAjaxChat.php?chat=" + chat + "&id=" + id,
    ...
    $(document).ready(function() {
        setInterval(function() {
            $("#conversation").load('NutritionistLoad.php?id=<?php echo $_GET["id"] ?>')
        }, 50);
    });
    </script>
